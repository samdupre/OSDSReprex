...
sf1 <- as(st_read("www/join_output_2.shp"), "Spatial")
...
server <- function(input, output, session) {
  ...
  secondPage_map_subtitle_topic <- reactive({
  ...
  output$secondPage_map_subtitle <- renderText({
    paste(
      i18n$t("Subtopic: "),
      secondPage_map_subtitle_topic())
  })
  ...
  output$first_page_map <- renderLeaflet({
    ...
    first_page_map_pal <- colorBin("YlGn", domain = r$sf2@data$Pop, bins = first_page_map_bins)
    
    first_page_map_labels <- sprintf(
      "<strong>%s </strong><br/>%s<br/>%g %s",
      r$sf2@data$NAME_1, r$sf2@data$NAME_2, r$sf2@data$Pop, i18n$t("people")) %>% 
    ...
    first_page_map_internal <- leaflet() %>%
      ...
      addLegend(pal=first_page_map_pal, 
                values = r$sf2@data$Pop, 
                opacity = 0.7, 
  ...
  output$second_page_map <- renderLeaflet({
    ...
    second_page_map_pal <- colorBin("YlOrRd", domain = r$sf2@data$SecondShow, bins = second_page_map_bins)
    
    second_page_map_labels <- sprintf(
      "<strong>%s </strong><br/>%s<br/>%g %s",
      r$sf2@data$NAME_1, r$sf2@data$NAME_2, r$sf2@data$SecondShow, i18n$t("percent prevalence, people with at least some difficulty")) %>% 
      lapply(htmltools::HTML)
    ...
    second_page_map_internal <- leaflet() %>%
      ...
      #                                                 bringToFront = TRUE),
      #             label = second_page_map_labels) %>%
      addLegend(pal=second_page_map_pal, 
                values = r$sf2@data$SecondShow, 
                opacity = 0.7, 
  ...
  observeEvent(input$walking_button, {
    ...
    r$sf2@data$SecondShow <- r$sf2@data$WalkRate
    ...
    leafletProxy("second_page_map", data = r$sf2) %>%
      clearMarkers() %>%
      clearShapes() %>%
      clearControls() %>%
  ...
  observeEvent(input$seeing_button, {
    ...
    r$sf2@data$SecondShow <- r$sf2@data$SeeRate
  ...
  observeEvent(input$hearing_button, {
    ...
    r$sf2@data$SecondShow <- r$sf2@data$HearRate
  ...
  observeEvent(input$cognition_button, {
    ...
    r$sf2@data$SecondShow <- r$sf2@data$CogRate
  ...
  observeEvent(input$selfcare_button, {
    ...
    r$sf2@data$SecondShow <- r$sf2@data$SelfcaRate
  ...
  observeEvent(input$communication_button, {
    ...
    r$sf2@data$SecondShow <- r$sf2@data$CommRate
  ...
  output$firstPage_Sex <- renderPlot({
    ...
    sexPlot <- dropdownFilteredData() %>%
      ...
      ggplot(aes(x = Type, y = Value, fill = Type, label = secondaryText)) +
  ...
  output$secondPage_Dis_ADM1Plot <- renderPlot({
    ...
    if (internal == "Walking") {
      plotTitle <- i18n$t("Prevalence Rate of Population with at Least Some Difficulty in Walking by Department, Guatemala 2018")
    } else if (internal == "Seeing") {
      plotTitle <- i18n$t("Prevalence Rate of Population with at Least Some Difficulty in Seeing by Department")
    } else if (internal == "Hearing") {
      plotTitle <- i18n$t("Prevalence Rate of Population with at Least Some Difficulty in Hearing by Department")
    } else if (internal == "Cognition") {
      plotTitle <- i18n$t("Prevalence Rate of Population with at Least Some Cognition Difficulty by Department")
    } else if (internal == "Selfcare") {
      plotTitle <- i18n$t("Prevalence Rate of Population with at Least Some Difficulty With Selfcare by Department")
    } else if (internal == "Communication") {
      plotTitle <- i18n$t("Prevalence Rate of Population with at Least Some Difficulty With Communication by Department")
    } else {
      plotTitle <- internal
    }
  ...
  page3_data <- imported_data %>% filter(Metric %in% c("Walking","Seeing","Hearing","Cognition","Selfcare","Communication"))
